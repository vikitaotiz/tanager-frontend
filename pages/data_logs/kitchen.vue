  ...
  <div>
      <v-card>
        ...
        <template v-slot:item.status="{ item }">
          <div v-if="item.timer <= 7">
            <v-chip color="primary" small>Success</v-chip>
          </div>
          <div v-if="item.timer > 7 && item.timer <= 15">
            <v-chip color="orange" small>Late</v-chip>
          </div>
          <div v-if="item.timer > 15">
...
methods: {
    ...
    fetchDataLogsKitchen(){
      this.$axios.get(`/timers_kitchen_all`).then(response => {
