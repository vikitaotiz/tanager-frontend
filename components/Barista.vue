  ...
  <div>
      <v-card
        ...
        <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-title><v-icon>mdi-glass-cocktail</v-icon> New Barista Orders</v-list-item-title>
            </v-list-item-content>
    ...
    <v-row no-gutters>
                ...
                >
                  <template v-slot:default="props">
                    <v-row>
                      ...
                      >
                        <v-card>
                          <v-list  v-if="timerT(product.timer) <= 3" dense dark color="primary">
                            <v-list-item>
                            ...
                            <v-list-item>
                               <v-btn text color="yellow" block small @click="openAddDialog(product)">Add Drink</v-btn>
                            </v-list-item>
                          </v-list>

                          <v-list v-if="timerT(product.timer) > 3 && timerT(product.timer) <= 8" dense dark color="orange">
                            <v-list-item>
                              <div><b> {{product.table}}</b> <small class="ml-2"><small>({{timerZ(product.timer)}})</small></small>
                                <v-divider color="white"></v-divider>
                                <small>Order Created By <b>{{product.user_name}}</b><br>
                          ...
                          <v-list v-if="timerT(product.timer) > 8" dense dark class="danger">
                            <v-list-item>
                              <div><b> {{product.table}}</b> <small class="ml-2"><small>({{timerZ(product.timer)}})</small></small>
                                <v-divider color="white"></v-divider>
                                <small>Order Created By <b>{{product.user_name}}</b><br>
                                ...
                                <template v-slot:default>
                                  <thead>
                                    <tr>
                                      ...
                                      <th class="text-left" v-if="product.content.length > 1" >Action</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                    <tr v-for="(item, index) in product.content" :key="index">
                                      <td>{{item.qty}}</td>
                                      <td>{{ item.name }}</td>
                                      <td><v-icon v-if="product.content.length > 1" dark color="red" @click="voidSingleItem(product, item, index)">mdi-delete</v-icon></td>
                                    </tr>
                            ...
                            <v-divider></v-divider>
                             ...
                             <v-card-actions>
                                ...
                                <v-btn
                                  ...
                                  v-if="loggedInUser.data.name === 'BARISTA'"
                                  small
                                  color="primary"
                                  text
          ...
          <v-col cols="12" sm="3" v-if="add_drink_table.length>0">

            <v-card outlined class="pa-2">

                <v-card color="primary" outlined v-for="(table, index) in add_drink_table" :key="index" class="mb-2">
            ...
            <v-col cols="12" sm="6">
              <v-card outlined class="pa-1">
                ...
                <v-text-field class="mt-2" v-model="added_products_search" label="Search for drinks..." outlined dense />
                          ...
                          <v-list dense class="v-card mb-1" v-for="(product, index) in filteredProducts(drinks)" :key="index">
                            <v-list-item>
                              <v-list-item-content>
...
export default {
  ...
  created(){
    ...
    socket.on('newOrderBarista', order=>{
      this.data = []
      this.fetchDrinks()
    })

    socket.on('additionalItem_to_barista_from_ready', order=>{
      this.add_drink_table = [];
      this.fetchAdddrinks();
    })

    socket.on('removePickedTableBarista', index => {
      this.data.splice(index, 1)
    })

    socket.on('AddItemsToBarista', new_data =>{
      this.data = [];
      this.fetchDrinks();
    })

     socket.on('single_item_deleted_barista', data1 =>{
      this.data = [];
      this.fetchDrinks();
    })
  },
  ...
  methods: {
    ...
    fetchDataLogsBarista(){
      this.$axios.get(`/timers_barista`).then(res => {
        ...
        this.data_logs_warning = res.data.data.filter(val => {
          return val.timer > 3 && val.timer <= 8;
        })
   ...
   openAddDialog(product){
    ...
    addItemsToTable(){
      ...
      this.$axios.patch(`/drinks/${this.added_product.id}`, data).then(res => {
        let new_data = res.data.data;
        socket.emit('AddItemsToBarista', new_data);
        this.closeDialogAdd();
    ...
    sendToAddPicks(table, index){
      ...
      .then(response => {
        ...
        this.$axios.delete(`/adddrinks/${table.id}`)
          .then(response => {
            this.$toast.success('Order sent to Pick!', {
              duration: 2000
            })
            this.loading = false
            this.add_drink_table.splice(index, 1)
          });
      })
    ...
    fetchDrinks(){
      ...
      if(this.loggedInUser.data.role === "Administrator" || this.loggedInUser.data.name === "BARISTA"){
        this.$axios.get(`/drinks`).then(response => {
    ...
    fetchProducts(){
      this.$axios.get(`/products`).then(response => {
          ...
          this.drinks = product_non_drinks
    ...
    fetchAdddrinks(){
      ...
      if(this.loggedInUser.data.role === "Administrator" || this.loggedInUser.data.name === "BARISTA"){
        this.$axios.get(`/adddrinks`).then(response => {
            response.data.data.forEach(val => {
              let table = {
                  id: val.id,
                  amount: val.amount,
              ...
              this.add_drink_table.push(table)
              });
          ...
          this.$axios.get(`/adddrinks`).then(response => {
            response.data.data.forEach(val => {
              ...
              this.add_drink_table = ar1.filter(el=>{
                return el.user_name === this.loggedInUser.data.name
              })
           })
           .catch(error => {
    ...
    sendOrderToPick(table, minutes){
      ...
      let data = {
        ...
        this.$axios.post(`/picks`, data).then(response => {
            ...
            // Remove barista data
            this.$axios.delete(`/drinks/${table.id}`).then(()=>{
              this.$toast.success('Order sent to Pick!', {
                duration: 2000
              })
              socket.emit('removePickedTableBarista', table_index)
              this.loading = false
    ...
    deleteSingleItem(){
      ...
      this.$axios.patch(`/drinks/${product.id}`, data).then(res => {
        socket.emit('single_item_deleted_barista', res.data.data);
        ...
        let data1 = {
          user_requesting_void: this.user_requesting_void,
          module: `${this.single_table_item_name} has been deleted from barista.`,
    ...
    deleteTable(){
      if(typeof this.void_id == 'number'){
        ...
        this.$axios.post('/loggers', data).then(res => {
          ...
          this.$axios.delete(`/drinks/${id}`)
            .then(res => {
                socket.emit('removePickedTable', table_index)
                this.data.splice(table_index, 1)
